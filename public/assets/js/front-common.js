...
$(document).on('submit', '.userFrm', function (event) {
    ...
    if (commonFormChecking(true, $(this).data('validation'))) {
                ...
                if (data.status) {
                    ...
                    if(data.data){
                        data.data.forEach(element => {
                            downloadURI(baseUrl+element,element);
                        });
                        if (data.redirect != '') {
...
$(document).on('click', '.addToCartSampleFile', function () {
    //alert('Test');
    var id = $(this).data('id');
    var dataJSON = {
        id: id,
        ...
        buttons: { */
            /* confirm: function () { */
                ...
                if (id) {
                    $.ajax({
                        type: "POST",
                        url: baseUrl + "add-sampleFile-to-cart",
                        data: dataJSON,
                        dataType: "JSON",
                        beforeSend: function () {
                            jc = $.dialog({
        ...
        buttons: { */
            /* confirm: function () { */
                ...
                if (id) {
                    $.ajax({
                        type: "POST",
                        url: baseUrl + "add-to-cart-from-wishlist",
                        data: dataJSON,
                        dataType: "JSON",
                        beforeSend: function () {
                            jc = $.dialog({
    ...
    $("." + cls).each(function () {
        if ($(this).hasClass("ckeditor")) {
            ...
            if ($.trim($(this).val()) == "") {
                ...
                if ($(this).attr("data-check") == "Email Id") {
                    var reg = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;
                    if (reg.test($.trim($(this).val())) == false) {
                        if (msgbox != "") {
                            $("." + msgbox).text("Enter valid Email address!!!");
